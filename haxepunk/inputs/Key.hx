 ...
 */
@:enum
abstract Key(Int) to Int from Int
{
	var ANY = -1;
	...
	public inline function toString():String
	{
		return switch (this) {
			...
			default:
				...
				{
					String.fromCharCode(char - Std.int(Key.A) + 65);
				}
				else if (char >= Std.int(Key.F1) && char <= Std.int(Key.F12))
				{
					"F" + Std.string(char - Std.int(Key.F1) + 1);
				}
				else if (char >= Std.int(Key.NUMPAD_1) && char <= Std.int(Key.NUMPAD_0))
				{
					"NUMPAD " + Std.string((char - Std.int(Key.NUMPAD_1) + 1)%10);
				}
				else if (char >= Std.int(Key.DIGIT_0) && char <= Std.int(Key.DIGIT_9))
				{
					Std.string(char - Std.int(Key.DIGIT_0));
				}
				else
				{
					"KEY (" + this + ")"; // maybe something better?
				}
		};
	}
